...
pub fn serialize(board_piece: Piece, positions: Vec<usize>) -> Result<BitBoard, GameErrors> {
    ...
    Ok(BitBoard {
        board: u64::from_str_radix(&result_string, 2).expect("Error converting to binary"),
        piece: board_piece,
    })
}

pub fn deserialize(board: BitBoard) -> Vec<usize> {
    let mut positions: Vec<usize> = vec![];
    let repeat_string = format!("{:b}", board.board);
    let string_board = format!("{}{:b}", "0".repeat(64 - repeat_string.len()),board.board);

    for (pos, bit) in string_board.chars().enumerate() {
        if bit == '1' {
            positions.push(63 - pos);
        }
    }
